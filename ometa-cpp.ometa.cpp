...
Snippet toSnippet(ometa::SourceView<std::string> src) {
...
int main(int argc, char* argv[]) {
	...
	const auto reference = identifier > ~"'"_L >= [](auto value){return "ometa::ptr("_S + value + ")"_S;};
	...
	const auto indexedValueRef = ~"$"_L > ometa::capture(+ometa::range(('0'), ('9'))) >= toSnippet >= [](auto value){return "ometa::pick<"_S + value + "-1>(value)"_S;};
	...
	*cpp = *(ometa::ptr(ruleForwardDecl)
	...
	const auto any = "."_L >= [](auto value){return "ometa::any()"_S;};
	const auto epsilon = "()"_L >= [](auto value){return "ometa::epsilon()"_S;};

	const auto character = ~"\\"_L > ~("n"_L | "r"_L | "t"_L | "\""_L | "\\"_L)
		| !"\\"_L > ~ometa::any();
	const auto literal = ometa::capture("\""_L > *(!"\""_L > character) > "\""_L) >= toSnippet > (ometa::epsilon() >= [](auto value){return "_L"_S;}) >= ometa::concat;
	...
	const auto parenthesized = ~"("_L > ~_ > ometa::ptr(choice) > ~_ > ~")"_L >= [](auto value){ return "("_S + value + ")"_S; };
	const auto capture = ~"<"_L > ~_ > ometa::ptr(choice) > ~_ > ~">"_L >= [](auto value){ return "ometa::capture("_S + value + ")"_S; };
	...
	const auto freeActionOrPredicate = ~"^"_L > ~_ > (ometa::epsilon() >= [](auto value){ return "(ometa::epsilon()"_S; }) > ((ometa::epsilon() >= [](auto value){return " >= "_S;}) > action
		| (ometa::epsilon() >= [](auto value){return " <= "_S;}) > predicate) > (ometa::epsilon() >= [](auto value){return ")"_S;}) >= ometa::concat;

	const auto boundActionOrPredicate = ~"->"_L > ~_ > ((ometa::epsilon() >= [](auto value){return " >= "_S;}) > action
		| (ometa::epsilon() >= [](auto value){return " <= "_S;}) > predicate) >= ometa::concat;
	...
	const auto sequence = prefix > *((~_ > ((ometa::epsilon() >= [](auto value){return " > "_S;}) > prefix >= ometa::concat
	...
	*ruleDefinition = identifier > ~_ > ~":="_L > ~_ > ometa::ptr(choice) > ~_ > ~";"_L >= [](auto value){ return "const auto "_S + ometa::pick<1-1>(value) + " = "_S + ometa::pick<2-1>(value) + ";"; };

	*ruleRedefinition = identifier > ~"'"_L > ~_ > ~"=>"_L > ~_ > ometa::ptr(choice) > ~_ > ~";"_L >= [](auto value){ return "*"_S + ometa::pick<1-1>(value) + " = "_S + ometa::pick<2-1>(value) + ";"; };
