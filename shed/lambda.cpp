...
template<typename TParent = Empty, typename Tag = Empty, typename TValue = Empty>
class Context {
...
public:
	...
	auto get() const {
		if constexpr (std::is_same_v<GetTag, Tag>) {
			...
			return parent.template get<GetTag>();
		}
	}
...
public:
	...
	MaybeMatch<source_type, value_type> parse(SourceView<source_type> src, auto ctx) {
