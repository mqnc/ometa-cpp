...
struct subrange_of {
	using type = subrange<decltype(std::declval<TInput>().begin())>;
};
template<forward_range TInput>
using subrange_of_t = subrange_of<TInput>::type;
...
using ParseResult = std::optional<Match<subrange_of_t<TInput>, TValue>>;
